...
class XReports(MethodView):
    ...
    @blp.alt_response(401, description="You should be the person who opened the shift to do this.")
    ...
    def post(self, shop_user_check_data):
        ...
        if shift["seller"]["id"] != shop_user_check_data["user_id"]:
            abort(401, message="Should be the person who opened the shift to create receipts.")

        receipts_cash_query = {"shop_id": shop_id, 
                               "datetime": {"$gt": shift["open_time"]}, 
                               "sell_type": "CASH"}
        result_receipts_cash = list(db.db.receipts.find(receipts_cash_query))

        receipts_card_query = {"shop_id": shop_id, 
                               "datetime": {"$gt": shift["open_time"]}, 
                               "sell_type": "CARD"}
        ...
        checks_cash_sum = Decimal(np.sum([receipt["sum"].to_decimal() for receipt in result_receipts_cash]))
        checks_card_sum = Decimal(np.sum([receipt["sum"].to_decimal() for receipt in result_receipts_card]))

        report = {"seller": shift["seller"], 
        ...
        db.db.reports.insert_one({**convert_decimal(report), "type": "X", "shop_id": shop_id})
    ...
    def get(self, shop_check_data):
        x_reports = list(db.db.reports.find({"type": "X", 
                                        "shop_id": shop_check_data["shop_id"]}))
...
class ZReports(MethodView):
    ...
    def post(self, shop_user_check_data):
        ...
        db.db.shifts.update_one({"shop_id": shop_id, "status": "opened"}, {"$set":  shift})

        db.db.reports.insert_one({**convert_decimal(report), "type": "Z", "shop_id": shop_id})
    ...
    def get(self, shop_check_data):
        z_reports = list(db.db.reports.find({"type": "Z", 
                                        "shop_id": shop_check_data["shop_id"]}))
