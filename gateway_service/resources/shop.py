...
from resources.utils import is_shop_owner, is_staff_member, send_request
from schemas import MessageOnlySchema, ShopEditSchema, ShopRegisterSchema, ShopSchema
...
class ShopRegister(MethodView):
    ...
    def post(self, shop_owner_data):
        ...
        result_shop_register = send_request(
            "POST", f"{SHOP_SERVICE_URL}/shop/register", data=shop_data
        )

        if result_shop_register["status_code"] != 201:
            return (
                result_shop_register["result_json"],
                result_shop_register["status_code"],
            )
        ...
        result_owner_register = send_request(
            "POST", f"{AUTH_SERVICE_URL}/register", data=owner_data
        )
        ...
        if result_owner_register["status_code"] != 201:
            ...
            return (
                result_owner_register["result_json"],
                result_owner_register["status_code"],
            )

        owner_id = result_owner_register["result_obj"]["id"]

        result_shop_to_owner = send_request(
            "POST", f"{SHOP_SERVICE_URL}/shop/{shop_id}/owner/{owner_id}"
        )
        ...
        if result_shop_to_owner["status_code"] != 200:
            ...
            return (
                result_shop_to_owner["result_json"],
                result_shop_to_owner["status_code"],
            )
...
class User(MethodView):
    ...
    @blp.alt_response(
        401, description="Requester doesn't belong to the requested shop."
    )
    ...
    def put(self, shop_edit_details, shop_id):
        ...
        result = send_request(
            "PUT", f"{SHOP_SERVICE_URL}/shop/{shop_id}", data=shop_edit_details
        )
        return result["result_json"], result["status_code"]
